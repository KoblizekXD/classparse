...
Method *GetMethodByName(ClassFile *cf, const char *name)
{
    for (uint16_t i = 0; i < cf->method_count; i++) {
        if (classparse_strcmp(cf->methods[i].name, name) == 0) return &cf->methods[i];
    }
    return NULL;
}
...
{
    for (uint16_t i = 0; i < cf->method_count; i++) {
        if (classparse_strcmp(cf->methods[i].name, name) == 0 && classparse_strcmp(cf->methods[i].descriptor, descriptor) == 0) return &cf->methods[i];
    }
    return NULL;
}
...
Field *GetFieldByName(ClassFile *cf, const char *name)
{
    for (uint16_t i = 0; i < cf->field_count; i++) {
        if (classparse_strcmp(cf->fields[i].name, name) == 0) return &cf->fields[i];
    }
    return NULL;
}
...
AttributeInfo *GetAttributeByName(AttributeInfo *attributes, uint16_t attribute_count, const char *name)
{
    for (uint16_t i = 0; i < attribute_count; i++) {
        if (classparse_strcmp(attributes[i].attribute_name, name) == 0) return &attributes[i];
    }
    return NULL;
}
...
{
    for (uint16_t i = 0; i < attribute_count; i++) {
        if (classparse_strcmp(attributes[i].attribute_name, name) == 0) return 1;
    }
    return 0;
}
...
char *GetInstructionName(uint8_t opcode)
{
    return "GetInstructionName is not support in freestanding mode";
}
